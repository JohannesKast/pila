//! Tests for `POST /profile/theme` (colour-theme preference handler).
...
use pila::handlers::jersey::{set_theme_post, SetThemeForm};
...
async fn call_set_theme(h: &Harness, theme: &str) -> axum::response::Response {
    set_theme_post(
        State(h.state.clone()),
        fake_user(h.user_id),
        CookieJar::new(),
...
#[tokio::test]
async fn set_theme_light_returns_ok_and_sets_cookie() {
    let h = build_harness().await;
    let res = call_set_theme(&h, "light").await;
    assert_eq!(res.status(), StatusCode::OK);
    let cookie = res
        .headers()
        .get("Set-Cookie")
        .expect("theme cookie must be set")
        .to_str()
        .unwrap();
    assert!(cookie.contains("pila_theme=light"), "got: {cookie}");
}

#[tokio::test]
async fn set_theme_dark_returns_ok() {
    let h = build_harness().await;
    let res = call_set_theme(&h, "dark").await;
    assert_eq!(res.status(), StatusCode::OK);
}
...
#[tokio::test]
async fn set_theme_unknown_returns_bad_request() {
    let h = build_harness().await;
    let res = call_set_theme(&h, "neon").await;
    assert_eq!(res.status(), StatusCode::BAD_REQUEST);
}

#[tokio::test]
async fn set_theme_empty_returns_bad_request() {
    let h = build_harness().await;
    let res = call_set_theme(&h, "").await;
    assert_eq!(res.status(), StatusCode::BAD_REQUEST);
}
...
#[tokio::test]
async fn set_theme_persists_choice_to_repo() {
    let h = build_harness().await;
    // New users default to dark.
    assert_eq!(
        h.users
