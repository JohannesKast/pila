...
//! picker. On a successful save we answer with `HX-Location: /` so HTMX does a
//! flash-free client-side navigation and the new tip name shows up in the
//! topbar and leaderboard immediately.
...
/// success we send `HX-Location: /` to re-render the page in place.
pub async fn profile_name_post(
    ...
    let sheet_err = |key: &str, name: String, real_name: String| {
        render_sheet(&state, &lang, name, real_name, t.get(key)).into_response()
    };
